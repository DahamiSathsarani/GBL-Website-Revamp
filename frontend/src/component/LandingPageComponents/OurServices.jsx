    ...
    <section className="w-[100%] h-[100%] bg-gradient-to-t from-[#00CEB1] to-[#1C3A7B] flex justify-center items-center pb-10 lg:pb-14 ">
      <div className="relative flex flex-col items-center justify-center w-full h-full">
        <div className="absolute top-0 left-0 w-full h-[150px] lg:h-[250px] z-0">
          <img
            className="w-[100%] h-[100%] lg:h-[80%] xl:h-[90%]"
            src={images.ShapeImage}
            alt="Shape-Image"
          />
          <motion.div
            className="absolute xs:top-4 top-4 w-[100%] py-5"
            initial={{ y: "-20%", opacity: 0 }}
            whileInView={{ y: 0, opacity: 1 }}
            transition={{ duration: 1.2, ease: [0.42, 0, 0.58, 1] }}
          >
            <h1 className="text-3xl font-bold text-center text-white xs:text-2xl lg:text-4xl xl:text-5xl ">
              {" "}
              What We Do ?
            </h1>
          </motion.div>
        ...
        <motion.div
          className="z-10 hidden gap-4 xs:px-[10%] px-8 lg:px-[5rem] xl:px-[6rem] 2xl:px-[8rem] lg:grid lg:grid-cols-4 "
        ...
        >
          {servicesData.map((service, index) => (
            <div
              key={index}
              className={`relative ${
                index === 0 ? "lg:mt-[5rem] xl:mt-[5rem]" : ""
              } 
                                    ${
                                      index === 1
                                        ? "lg:mt-[9rem] xl:mt-[9rem]"
                                        : ""
                                    } 
                                    ${
                                      index === 2
